...
internal sealed class Phase35IEvidenceBuilder
{
    internal Phase35IEvidence Build(Phase35IContainmentResult result, Phase35HRequest request, Phase35IWorkerProfile profile, Phase35IRunnerIdentity runner, Phase35IIsolationEvidence isolation, Phase35IJobEvidence job)
    {
        ...
        var payloadObject = new { schemaVersion = Phase35IContracts.ContractVersion, result, requestHash = Phase35HAuthentication.Hash(request), profile, runner, containmentProfile = Phase35IContracts.ContainmentProfileVersion, restrictedToken = token, job, isolation, phase35HAuditCorrelationHash = result.Phase35HAuditCorrelationHash, proofStatus = OperatingSystem.IsWindows() && result.JobAssigned ? Phase35IProofStatus.ProvenForInertWorkload : Phase35IProofStatus.PartiallyProven };
