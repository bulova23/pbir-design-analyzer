...
describe('ConceptStudioView', () => {
  it('blocks concept generation until the brief is approved', () => {
    ...
    const onSubmitBaselineForApproval = jest.fn();
    ...
    render(
      <ConceptStudioView
        ...
        onSubmitBaselineForApproval={onSubmitBaselineForApproval}
  ...
  it('supports alternate concept comparison and explicit baseline selection without materialization controls', () => {
    ...
    expect(screen.getByRole('button', { name: 'Submit Baseline For Approval' })).toBeEnabled();
    expect(onApproveBaseline).not.toHaveBeenCalled();

    fireEvent.click(screen.getByRole('button', { name: 'Submit Baseline For Approval' }));

    expect(onSubmitBaselineForApproval).toHaveBeenCalledTimes(1);
    expect(screen.getByText('Concept approval: Pending approval')).toBeInTheDocument();
    expect(screen.getByText('Approve the concept baseline to unlock Draft Studio.')).toBeInTheDocument();
    expect(screen.getByRole('button', { name: 'Approve Concept Baseline' })).toBeEnabled();
    ...
    expect(screen.getByText('Concept approval: Approved')).toBeInTheDocument();
