...
export class AnthropicVisualAuditProvider implements VisualAuditProvider {
  readonly providerName = 'Anthropic Claude Vision';
  ...
  async analyzeCapture(input: VisualAuditInput): Promise<VisualAuditFinding[]> {
    ...
    if (!apiKey) {
      throw new Error(
        'Anthropic API key not configured. Run "PBIR Design Analyzer: Configure Audit Provider" to add your key.',
      );
    }
