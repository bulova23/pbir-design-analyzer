...
describe('extractSemanticModelEvidence', () => {
  ...
  it('extracts bounded analytics-facing semantic model usage evidence', () => {
    ...
    const evidence = extractSemanticModelEvidence(tempDir);
  ...
  it('returns no evidence when no semantic model artifacts are present', () => {
  ...
  it('caps extracted evidence to a bounded number of signals', () => {
    fs.mkdirSync(path.join(tempDir, 'src', 'data'), { recursive: true });
    fs.writeFileSync(
