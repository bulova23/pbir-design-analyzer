...
Phase 35B closes the immediate composition gap between the Phase 35A governance contracts and a future provider by adding focused runtime services rather than expanding `PbirScoringService`. The remaining high-risk gaps are intentionally deferred: provider sandbox/trust attestation, credential isolation, durable tamper-evident audit, output scanning corpus, provider conformance testing, replay ledger/resource quotas, and real-provider output validation. These are prerequisites for Phase 35C and are not solved by the offline fake path.
